      ...
      <Link href={`product/${product.node._meta.uid}`}>
        <a className={`flex ${product.node.style_new ? "h-full justify-end relative" : "flex-col"}`}>
          {product.node.price_sale &&
            <Button className="px-p4 absolute top-p4 right-p4" rounded="rounded" type="text" text="sale" color="text-white" fontFamily="font-montserrat" bg="bg-blueless" fontSize="text-xss"/>
          }         
          <div className={`flex ${product.node.style_new ? "m-0 h-full" : "h-2/4 my-4 mx-auto"}`}>
            ...
            {product.node.style_new &&
              <Button className="px-2 mb-4" rounded="rounded" type="text" text="new" color="text-blueless" fontFamily="font-montserrat" bg="bg-white" fontSize="text-xss"/>
            }
            {product.node.style_new &&
             <span className="uppercase text-white tracking-widest font-montserrat400 text-sm my-2">LifeStyle</span>
            }
            <span className={`${product.node.style_new ? "text-white font-playfair700 text-3xl mb-2" : "text-gray-700 font-montserrat400 text-xs"}`}>{product.node.name}</span>
            {product.node.style_new &&
              <div className="rounded-2xl bg-white py-4 mt-2 flex justify-center items-center divide-x">
                <span className="text-blueless font-montserrat400 text-sm pr-2">{`$ ${product.node.price_regular} USD`}</span>
                <Button className="px-2 uppercase"type="text" text="By Now" color="text-blueless" fontFamily="font-montserrat" bg="bg-white" fontSize="text-sm"/>                
              </div>
            }
            {!product.node.style_new &&
