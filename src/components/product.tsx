      ...
      <Link href={`product/${product.node._meta.uid}`}>
        <a className={`flex ${product.node.style_new ? "h-full justify-end relative" : "flex-col"}`}>
          ...
          <div className={`flex ${product.node.style_new ? "m-0 h-full" : "w-2/4 h-2/4 my-4"}`}>
            ...
            {product.node.style_new &&
             <span className="uppercase text-white font-montserrat text-xss my-2">LifeStyle</span>
            }
            <span className={`${product.node.style_new ? "text-white font-playfair700 text-2xl mb-2" : "text-gray-700 font-montserrat400 text-xs"}`}>{product.node.name}</span>
            {product.node.style_new &&
